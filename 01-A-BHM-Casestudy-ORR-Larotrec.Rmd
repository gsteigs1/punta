...
Note: inconsistency in ERG report page 66: "The prior distribution for  was centred around a probability of 0.3 (a log-odds of -1.3863)".
-> logit(0.3) = -0.847
...
p_better_re <- get_bin_threshold(fit_re, thresholds = c(0.2, 0.3, 0.5), 
...
```{r, results='asis'}
pander::pandoc.table(p_better_re, row.names = FALSE, split.tables = Inf, justify = "llll", digits = 3)
```
...
## threshold probabilities
p_better_fe <- get_bin_threshold(fit_fe, thresholds = c(0.2, 0.3, 0.5), 
                                 node.stem = "p", node.extras = c(".pop", ".new"), 
                                 study.labs = dorr_erg$`Tumour type`)
...
```{r, results='asis'}
pander::pandoc.table(p_better_fe, row.names = FALSE, split.tables = Inf, justify = "llll", digits = 3)
```
...
## threshold probabilities
p_better_re_hn <- get_bin_threshold(fit_re_hn, thresholds = c(0.2, 0.3, 0.5), 
                                    node.stem = "p", node.extras = c(".pop", ".new"), 
                                    study.labs = dorr_erg$`Tumour type`)
...
```{r, results='asis'}
pander::pandoc.table(p_better_re_hn, row.names = FALSE, split.tables = Inf, justify = "llll", digits = 3)
```
