...
# Cumulative meta-analysis (ERG model, full data)
## Background
...
for(i in 1:n_cma){
  ...
  for(j in 1:min(n_shrink, i + n_skip)){
    ...
    out_ij <- data.frame("cma_id" = i,
                         ...
                         "est_grp_med_ij" = est_grp_med_ij,
                         "est_grp_mean_ij" = est_grp_mean_ij,
...
##### Shrunken group level estimates (fully Bayesian)
__Figure__ Shrunken group level estimates: posterior medians
```{r}
ggplot(data = dshrinkage) +
  geom_line(aes(x = n_tissues, y = est_grp_med_ij, color = tissue)) +
  #ylim(-0.25, 1.25) +
  scale_x_continuous(breaks = 3:14) +
  labs(y = "Shrunken estimates (logit scale)",
       x = "No tissues") +
  theme_bw() +
  theme(legend.title = element_blank())
```

__Figure__ Shrunken group level estimates: posterior means
```{r}
ggplot(data = dshrinkage) +
  geom_line(aes(x = n_tissues, y = est_grp_mean_ij, color = tissue)) +
  #ylim(-0.25, 1.25) +
  scale_x_continuous(breaks = 3:14) +
  labs(y = "Shrunken estimates (logit scale)",
       x = "No tissues") +
...
# Reduced cumulative meta-analysis: impact of low information studies on vague prior (ERG model)
## Background
Redo analysis above, but only include studies no 8 onwards.

The idea is to assess how the analysis changes compared to the prior when only such low information studies are used.
...
Fit cumulative MA models, but add only the following data (study no 8 onwards).

```{r}
dorr_red <- dorr_erg[8:14,]
dorr_red
```

This time, do the cumulative MAs adding studies one-by-one. Collect and report results from the first study on, i.e. to not impose a minimum of three studies. Interest will only be in the estimation of heterogeneity.
...
for(i in 1:nrow(dorr_red)){
  dorr_red_i <- dorr_red[1:i,]
  djags_i <- list(ns = nrow(dorr_red_i),
                  r = dorr_red_i$x,
                  n = dorr_red_i$n)
...
### Posterior summaries of RE SD

```{r}
# Initialize data frames to store results
cma_resd <- as.data.frame(matrix(NA, # random effect SD (tau)
                                 nrow = nrow(dorr_red), ncol = 8, 
                                 dimnames = list(NULL, c("n_tissues", "n_tot", 
                                                         "dn", "dr",
                                                         "mean", "sd",
                                                         "IQR",   "CrIR"))),
...
for (i in 1:length(cma_fits)){
  fit_re_i <- cma_fits[[i]]
  dorr_red_i <- dorr_red[1:i,]
  dsums_i <- c(n_tissues = i,
               n_tot = sum(dorr_red_i$n),
               dn    = tail(dorr_red_i$n, 1),
               dr    = tail(dorr_red_i$x, 1)
               #r_tot = sum(dorr_red_i$x)
               )
  ...
  rm(fit_re_i, dorr_red_i)
}

```


__Table__ Reduced cumulative MA results: posterior summaries re.sd (low information studies, vague prior)
```{r, results='asis'}
pander::pandoc.table(cma_resd, row.names = FALSE, split.tables = Inf, justify = "left", digits = 2)
```
...
# Reduced cumulative meta-analysis: impact of low information studies on informative prior (HN(0.5))
## Background
Redo analysis again, but now with informative prior.

The hypothesis is that low information data points have no impact of vague prior (as seen above), but may have impact on informative prior.

## Data and "procedure"

Same as above, studies no. 8 onwards.
```{r}
dorr_red <- dorr_erg[8:14,]
dorr_red
```
...
for(i in 1:nrow(dorr_red)){
  ...
  fit_re_i <- jags(data = c(djags_i, my_priors2),
                   n.chains = n_chains,
                   n.iter = n_iter,
                   n.burnin = n_burnin,
                   n.thin = n_thin,
...
__Table__ Cumulative MA results: posterior summaries re.sd (low information studies, but informative prior)
```{r, results='asis'}
pander::pandoc.table(cma_resd, row.names = FALSE, split.tables = Inf, justify = "left", digits = 2)
```

Compare this with the HN(sigma=0.5) prior summaries:
__Table__ Summaries of HN(sigma=0.5) prior
```{r, results='asis'}

hn_sum <- get_hn_sum(sigma = 0.5)

dhnprior <- data.frame(mean = hn_sum['mean'],
                      sd = hn_sum['sd'],
                      IQR = hn_sum[6] - hn_sum[4],
                      CrIR = hn_sum[7] - hn_sum[3])

pander::pandoc.table(dhnprior, row.names = FALSE, split.tables = Inf, justify = "left", digits = 3)
```
